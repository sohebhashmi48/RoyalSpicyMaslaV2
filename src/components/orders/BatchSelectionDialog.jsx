...
export default function BatchAllocationDialog({ isOpen, onClose, order }) {
  ...
  // Debug: Log the order prop when component mounts or changes
  console.log('DEBUG: BatchAllocationDialog received order prop:', order);

  const itemsNeedingAllocation = useMemo(() => {
    const items = orderData?.items || [];
    const allocationItems = [];

    console.log('DEBUG: Processing items for allocation:', items);

    for (const item of items) {
      console.log('DEBUG: Processing item:', item);
      
      // Skip mix container items - we only want to allocate their components
      if (item.source === 'mix-calculator') {
        console.log('DEBUG: Found mix item:', item);
        let customDetails;
        try {
          customDetails = typeof item.custom_details === 'string'
            ? JSON.parse(item.custom_details)
            : item.custom_details;
          console.log('DEBUG: Parsed custom_details:', customDetails);
        } catch (e) {
          console.error('Error parsing custom_details for mix item:', e);
          continue;
        }

        // Extract individual products from mix
        if (customDetails && customDetails.mixItems && Array.isArray(customDetails.mixItems)) {
          console.log('DEBUG: Processing mixItems:', customDetails.mixItems);
          customDetails.mixItems.forEach((mixItem, index) => {
            console.log('DEBUG: Processing mixItem:', mixItem);
            ...
            if (productId) {
              allocationItems.push({
                ...
                original_price: parseFloat(mixItem.price || 0),
                allocated_cost: parseFloat(mixItem.actualCost || mixItem.allocatedBudget || 0)
              });
              console.log('DEBUG: Added allocation item for mix component:', {
                product_id: productId,
                product_name: mixItem.name,
                quantity: parseFloat(mixItem.calculatedQuantity || mixItem.quantity || 0)
              });
            } else {
              console.warn('DEBUG: No valid productId found for mixItem:', mixItem);
            }
          });
        } else {
          console.warn('DEBUG: No valid mixItems found in custom_details:', customDetails);
        }
      }
    
      // Handle regular items with product_id (excluding mix containers)
  ...
  const autoAllocateForItem = (item) => {
    ...
    console.log('DEBUG: Auto-allocation result:', {
  ...
  useEffect(() => {
    const ensureOrder = async () => {
      ...
      console.log('DEBUG: ensureOrder called with order:', order);
      
      if (order?.items && order.items.length) {
        console.log('DEBUG: Using order items directly');
        setOrderData(order);
        return;
      }
      
      if (order?.id) {
        console.log('DEBUG: Fetching order details for ID:', order.id);
        try {
          setLoading(true);
          const res = await fetch(`http://localhost:5000/api/orders/${order.id}`);
          const data = await res.json();
          console.log('DEBUG: Order fetch response:', data);
          
          if (data.success) {
            console.log('DEBUG: Setting order data with items:', data.data.items?.length || 0, 'items');
            setOrderData(data.data);
          } else {
            console.error('DEBUG: Order fetch failed:', data.message);
            showError(data.message || 'Failed to load order details');
          }
        } catch (e) {
          console.error('DEBUG: Order fetch error:', e);
          showError('Failed to load order details');
        } finally {
          setLoading(false);
        }
      } else {
        console.warn('DEBUG: No order ID or order is null');
      }
    };
    ensureOrder();
  ...
  useEffect(() => {
    ...
    const fetchBatches = async () => {
      try {
        setLoading(true);
        console.log('DEBUG: Starting batch fetch for items:', itemsNeedingAllocation);
        const all = {};
        
        for (const item of itemsNeedingAllocation) {
          console.log(`DEBUG: Fetching batches for product_id: ${item.product_id}, product_name: ${item.product_name}`);
          try {
            const res = await fetch(`http://localhost:5000/api/inventory/product/${item.product_id}/batches`);
            const data = await res.json();
            console.log(`DEBUG: Batch fetch response for product ${item.product_id}:`, data);
            ...
            if (data.success && data.data.length > 0) {
              console.log(`DEBUG: Found ${data.data.length} batches for product ${item.product_id}`);
            } else {
              console.warn(`DEBUG: No batches found for product ${item.product_id}`);
            }
          } catch (fetchError) {
            console.error(`DEBUG: Error fetching batches for product ${item.product_id}:`, fetchError);
            all[item.product_id] = [];
          }
        }
        
        console.log('DEBUG: All fetched batches:', all);
        setAvailableBatches(all);

        // Fetch existing allocations
        if (order?.id) {
          console.log('DEBUG: Fetching existing allocations for order:', order.id);
          try {
            const res2 = await fetch(`http://localhost:5000/api/orders/${order.id}/allocations`);
            const data2 = await res2.json();
            console.log('DEBUG: Existing allocations response:', data2);
            ...
            if (data2.success && Array.isArray(data2.data) && data2.data.length) {
              ...
              console.log('DEBUG: Grouped existing allocations:', grouped);
            } else {
              setAllocations({});
              console.log('DEBUG: No existing allocations found');
            }
          } catch (allocError) {
            console.error('DEBUG: Error fetching existing allocations:', allocError);
            setAllocations({});
          }
        }
      } catch (e) {
  ...
  const groupedItems = useMemo(() => {
    ...
    // Create mix components section - each mix item as individual
    if (mixItems.length > 0) {
      groups['mix_components'] = {
        type: 'mix_components',
        title: 'Mix Components',
        description: 'Individual components from mixes that need batch allocation',
        items: mixItems
      };
    }
              ...
              {groupedItems.map((group, groupIndex) => (
                <div key={groupIndex} className="space-y-4">
                  ...
                  {group.items.map((item) => {
                    ...
                    console.log('DEBUG: Displaying item', {
                      product_name: item.product_name,
                      product_id: item.product_id,
                      batches: batches,
                      itemAllocs: itemAllocs,
                      allocated: allocated,
                      remaining: remaining,
                    ...
                    return (
                      <div key={item.order_item_id} className={`border rounded-lg p-5 bg-white shadow-sm ${
                        group.type === 'mix_components' ? 'border-purple-200 ml-4' : 'border-gray-200'
                      }`}>
